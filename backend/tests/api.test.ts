 ...
 *
 * Uses a pre-seeded WorkflowCache (no chain, no model download).
 * HTTP client: Node http.createServer (same pattern as gateway tests).
 */
...
// Fixed embedder: "defi" → [1,0,0], "data" → [0,1,0], others → [0,0,1]
...
beforeAll(async () => {
	...
	const reader = new RegistryReader() // not configured → demo mode
...
describe('POST /api/workflows/list', () => {
	const VALID_PAYLOAD = {
		workflowId: 'wf_new_01',
		pricePerInvocation: '7500',
		description: 'A brand new workflow for testing',
		detailedDescription: 'Very detailed description of this new workflow.',
		category: 'compute',
		creatorAddress: '0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC',
		inputs: [{ name: 'param1', fieldType: 'string', description: 'A param', required: true }],
		outputs: [{ name: 'result', fieldType: 'number', description: 'A result', required: true }],
	}

	test('returns 201 with created workflow on valid input', async () => {
		const res = await fetchApp(app, '/api/workflows/list', {
			method: 'POST',
		...
		expect(wf.workflowId).toBe('wf_new_01')
		expect(wf.pricePerInvocation).toBe('7500')
	})

	test('returns 400 on missing required field', async () => {
		const res = await fetchApp(app, '/api/workflows/list', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ ...VALID_PAYLOAD, workflowId: undefined }),
		})
		expect(res.status).toBe(400)
	})

	test('returns 400 for invalid workflowId (uppercase)', async () => {
		const res = await fetchApp(app, '/api/workflows/list', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ ...VALID_PAYLOAD, workflowId: 'WF_UPPER' }),
		})
		expect(res.status).toBe(400)
	})

	test('returns 400 for description over 160 chars', async () => {
		const res = await fetchApp(app, '/api/workflows/list', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ ...VALID_PAYLOAD, workflowId: 'wf_long_desc', description: 'x'.repeat(161) }),
		})
		expect(res.status).toBe(400)
	})

	test('returns 409 when workflowId already exists', async () => {
		const res = await fetchApp(app, '/api/workflows/list', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ ...VALID_PAYLOAD, workflowId: 'wf_hf_monitor_01' }),
		})
	...
	test('returns 400 for invalid category', async () => {
		const res = await fetchApp(app, '/api/workflows/list', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ ...VALID_PAYLOAD, workflowId: 'wf_bad_cat', category: 'invalid' }),
		})
		expect(res.status).toBe(400)
	})

	test('returns 400 for invalid creatorAddress', async () => {
		const res = await fetchApp(app, '/api/workflows/list', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ ...VALID_PAYLOAD, workflowId: 'wf_bad_addr', creatorAddress: 'not-an-address' }),
		})
		expect(res.status).toBe(400)
	})
})
