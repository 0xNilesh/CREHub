...
// node_modules/@chainlink/cre-sdk/dist/sdk/utils/decode-json.js
...
var inputSchema = exports_external.object({
  walletAddress: exports_external.string().regex(/^0x[0-9a-fA-F]{40}$/, "Must be a valid EVM address"),
  protocol: exports_external.enum(["aave", "compound"]).default("aave")
});
var computeRiskLevel = (healthFactor) => {
  if (healthFactor >= 1.5)
    return "safe";
  if (healthFactor >= 1.1)
    return "warning";
...
var fetchHealthFactor = (sendRequester, config, input) => {
...
var onHTTPTrigger = (runtime2, payload) => {
  runtime2.log("CREHub HTTP trigger received");
  const rawInput = decodeJson(payload.input);
  const input = inputSchema.parse(rawInput);
  runtime2.log(`Input: walletAddress=${input.walletAddress} protocol=${input.protocol}`);
  const httpClient = new ClientCapability;
  const result = httpClient.sendRequest(runtime2, fetchHealthFactor, consensusIdenticalAggregation())(runtime2.config, input).result();
  runtime2.log(`Output: healthFactor=${result.healthFactor} riskLevel=${result.riskLevel}`);
  return result;
};
