...
init_toHex();
init_keccak256();
function encodeReport(workflowId, resultHash) {
  return encodeAbiParameters(parseAbiParameters("string workflowId, bytes32 resultHash"), [workflowId, resultHash]);
}
...
var onHTTPTrigger = (runtime2, payload) => {
  ...
  runtime2.log("[on-chain] Encoding workflow report...");
  const resultHash = hashOutput(output);
  const reportPayload = encodeReport(runtime2.config.workflowId, resultHash);
  runtime2.log("[on-chain] Generating CRE report...");
  const reportResponse = runtime2.report(prepareReportRequest(reportPayload)).result();
  runtime2.log(`[on-chain] Writing to CREHubExecutor on Sepolia: ${runtime2.config.executorAddress}`);
  const sepoliaNetwork = getNetwork({
    chainFamily: "evm",
    chainSelectorName: runtime2.config.chainSelectorSepolia,
    isTestnet: true
  });
  if (!sepoliaNetwork) {
    throw new Error(`[on-chain] Unknown chain: ${runtime2.config.chainSelectorSepolia}`);
  }
  const evmClient = new ClientCapability(sepoliaNetwork.chainSelector.selector);
  const writeResult = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.executorAddress,
  ...
  runtime2.log(`[on-chain] ✓ Transaction confirmed: ${txHash}`);
  return { ...output, onChainTxHash: txHash };
};
var initWorkflow = (config) => {
  const httpCapability = new HTTPCapability;
  return [
    handler(httpCapability.trigger({
      authorizedKeys: [
        {
