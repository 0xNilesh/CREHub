...
var buildGreeting = (input) => ({
  message: `Hello, ${input.name ?? "World"}!`,
  timestamp: new Date().toISOString()
});
var onHTTPTrigger = (runtime2, payload) => {
  runtime2.log("CREHub hello-world trigger received");
  const rawInput = decodeJson(payload.input);
  const input = inputSchema.parse(rawInput);
  runtime2.log(`Input: name=${input.name ?? "(none)"}`);
  const output = buildGreeting(input);
  runtime2.log(`Output: message="${output.message}" timestamp="${output.timestamp}"`);
  return output;
};
